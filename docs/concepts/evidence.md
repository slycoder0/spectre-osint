...
O **SPECTRE OSINT** foi concebido sob princípios rigorosos de disciplina probatória: **dados públicos observados são registrados com exatidão técnica, a proveniência de cada campo é preservada, e identidades nunca são presumidas a partir de respostas parciais.**
...
Pistas fornecidas pelo investigador (`--name`, `--email`, `--alias`, `--website`) são armazenadas estritamente com proveniência de operador (`source="user"`). Elas representam hipóteses investigativas e permanecem separadas dos dados observados diretamente na web (`source="observed"` ou `source="platform_api"`).
