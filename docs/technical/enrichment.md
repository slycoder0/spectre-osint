...
| `derived_from` | Campo de origem de um valor derivado. Hoje apenas `personal_domain`, com `derived_from="website"`. O cálculo de `personal_domain` não mudou. **`derived_from` e `DERIVED` são duas metades da mesma afirmação**, validadas nos dois sentidos: uma observação derivada precisa nomear sua origem, e só uma observação derivada pode nomear uma. Vale na linha e no item. O token não é interpretado — nenhum vocabulário de origem futura está fixado no código — mas precisa ser não vazio. |
