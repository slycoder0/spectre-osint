...
- **O parser é total sobre o transporte canônico que este contrato emite.** `parse_observed()` lê todo `observed_at` em texto — na linha e em cada item — **uma vez** com `datetime.fromisoformat()` antes da validação Pydantic, e repassa um valor com fuso como o objeto `datetime` já interpretado em vez da string original. O motivo é que os dois parsers não aceitam as mesmas grafias: `datetime.isoformat()`, o serializador deste contrato, escreve um deslocamento com segundos ou fração de segundo como `+00:00:30` / `+00:00:30.123456`, a biblioteca padrão relê essas grafias e o campo `AwareDatetime` aceita o objeto equivalente, mas o parser de **string** do Pydantic as recusa. Um `ObservedField` válido podia então emitir um transporte que o seu próprio parser rejeitava — construir → `to_transport()` → `parse_observed()` falhava. Fechado o buraco, o transporte é estável byte a byte: `to_transport()` → `parse_observed()` → `to_transport()` devolve a mesma grafia, no caminho escalar e no caminho de `items`. **Nada é normalizado para UTC:** o objeto conserva seu `tzinfo`, o valor do deslocamento e a precisão de microssegundos, não há `astimezone()` envolvido, e um timestamp cujo instante equivalente em UTC cairia fora dos anos 1–9999 (`0001-01-01T00:00:00+00:00:30`, `9999-12-31T23:59:59-00:00:30`) segue representável — `_instant_key()` não mudou. Como a porta delega o domínio de grafias à biblioteca padrão, ela é um pouco mais tolerante na **entrada** (`+00`, segundos de deslocamento acima de 59 são aceitos e reescritos na forma canônica); nenhuma dessas grafias é emitida por `isoformat()`, então nenhum transporte escrito pelo SPECTRE muda. Uma string de timestamp **inválida** continua recusada: texto ilegível segue texto, o Pydantic continua dono do erro, e nada é reparado, re-estampado como "agora" ou presenteado com um fuso que não foi observado. Este passo é **endurecimento de compatibilidade anterior a B2-03B** — ele não torna o parser autoritativo em nenhum consumidor.
- **Um deslocamento UTC puramente subsegundo é restaurado a partir da grafia do transporte.** `datetime.isoformat()` escreve um deslocamento sem horas, minutos nem segundos inteiros como `+00:00:00.ffffff`, e o acelerador em C de `datetime.fromisoformat()` no Python 3.12 decide pelo UTC olhando só a parte de segundos inteiros: o deslocamento é descartado em silêncio ([CPython gh-152079](https://github.com/python/cpython/issues/152079), corrigido no 3.13 e posteriores; o 3.12 é suportado aqui e segue afetado). `2026-01-01T00:00:00+00:00:00.000001` voltava como `2026-01-01T00:00:00+00:00` — um **instante diferente**, a um microssegundo de distância, e portanto uma afirmação de proveniência que este contrato nunca fez. Por isso `_as_aware()` recupera o deslocamento assinado que o próprio transporte declara, em microssegundos inteiros lidos dos seus caracteres, e o repõe **antes da validação Pydantic** quando a interpretação não o preservou. Só o rótulo `tzinfo` é substituído, com `replace(tzinfo=...)`: a interpretação afetada lê corretamente os campos do relógio local e apenas os rotula mal como UTC, então repor o rótulo devolve o instante que o transporte nomeou — `astimezone()` converteria o instante já errado e carregaria a corrupção adiante. **Nenhuma conversão de instante e nenhuma normalização para UTC acontecem**, num runtime afetado ou não; num runtime corrigido a interpretação já concorda com a grafia e nada é substituído. A restauração fica **atrás** de uma leitura bem-sucedida da biblioteca padrão, sobre um valor já com fuso, então ela corrige um deslocamento mas nunca torna válido um timestamp inválido, e não amplia a aceitação: uma fração zero (`+00:00:00.000000`, que `isoformat()` não emite) declara deslocamento zero e não inventa nenhum, e um deslocamento com segundos inteiros não nulos (`+00:00:30.123456`) não é este defeito e segue inteiramente com a biblioteca padrão. Com isso o transporte canônico faz round-trip idêntico **no Python 3.12 e no 3.13**. Como o passo anterior, isto é **endurecimento de compatibilidade anterior a B2-03B**: nenhum consumidor passa a tratar o modelo como autoritativo.
- **B2-03B1: se `observed` está presente, o modelo validado é a autoridade a jusante.** `records_from_findings()`, `observed_profile_fields()` e, por consequência, `extract_indicators()` leem o canal observado através de um único ponto de validação — `read_observed()`, em `observed.py`. A decisão é por **presença da chave**, nunca por veracidade e nunca por tipo. `observed` **ausente** é uma linha verdadeiramente legada e mantém o fallback de nível superior intacto. `observed` **presente** — válido, `{}`, `None`, `[]` ou malformado — significa que o canal autoritativo existe, e as chaves de nível superior **deixam de ser evidência** para os atributos que ele cobre: um `display_name` de nível superior em conflito perde, e `{}` é enriquecimento vazio e autoritativo, não um convite ao fallback. Transporte presente e **inválido** falha fechado — nenhum atributo observado é exposto e **não há fallback** — mas o registro do perfil sobrevive com `platform`, `username`, `profile_url`/`final_url`, `check_status` e `entity_id`, porque `PERFIL EXISTE != MESMA PESSOA` e um payload de enriquecimento quebrado não desfaz a verificação do perfil público. O `Finding.data` original nunca é reparado nem reescrito: a autoridade é decidida na **leitura**, sem migração e sem validação no momento da gravação, de modo que um achado vindo do cache ou do banco recebe exatamente a mesma validação que um ao vivo. `IdentityRecord.provenance` passa a ser a visão **validada** (`parse_observed(...).to_transport()`) para dados modernos válidos, e um mapeamento vazio quando o transporte é inválido.
- **`rejected_by` presente é evidência inativa a jusante (B2-03B1).** O teste é `is not None`, não veracidade: um token de rejeição vazio, ainda que válido para o modelo, continua sendo uma rejeição. Um campo rejeitado não vira atributo ativo do `IdentityRecord`, não entra em `links`, não é apresentado como linha observada ativa, não se torna indicador de pivô e não é explicado como evidência — nem como sinal **positivo** (`same_display_name`, `same_organization`, …) nem como **conflito** (`distinct_display_name`, …). Um valor rejeitado não é um valor que pontua zero: ele está ausente da comparação. A observação rejeitada **permanece** no transporte original e na proveniência validada. O produtor **ainda não emite** `rejected_by` — isso é B2-03B2.
- **A forma semântica é verificada no consumidor (B2-03B1).** `ObservedField.value` é genérico de propósito, mas um consumidor sabe o que um nome de exibição é: campos escalares (`display_name`, `username`, `bio`, `location`, `organization`, `website`, `personal_domain`, `public_email`, `avatar_url`, `public_id`) com valor de lista, e campos de lista (`external_links`, `social_links`) com valor escalar, são **suprimidos** em vez de convertidos — `["Alice"]` nunca vira `"['Alice']"`, e um `social_links` escalar não vira link só porque caberia numa lista. Suprime-se apenas aquele campo: um mapeamento válido no modelo não é recusado inteiro por causa de um campo incompatível, e nomes de campo desconhecidos não são proibidos — compatibilidade futura não é escopo desta fatia.
- **Links em B2-03B1 são os valores validados da linha, e a explicação segue grosseira.** `IdentityRecord.links` vem de `external_links` e `social_links` aceitos, nessa ordem, deduplicados sem reescrever nenhuma grafia; `public_links` de nível superior é ignorado quando `observed` existe, e `website` continua sendo atributo próprio (`_link_points_at()` já o considera à parte). O valor da linha é seguro como pertencimento porque o contrato B2-03A garante que ele é a projeção verdadeira dos `items`. Isto **não** afirma que o `source` de linha (`"multiple"`) seja a origem de cada membro: `items` continua preservado, e ensinar a correlação a identificar **qual `ObservedItem`** sustentou um `cross_profile_link` é B2-03B3. Nenhum peso, conflito, banda ou limiar de clusterização mudou.
