...
- **`ObservedField` modela a distinção `atributo observado != atributo civil verificado`.** O modelo não *garante* por si que todo consumidor a imponha; B2-03A introduziu o contrato e B2-03B1 passou a impô-lo nos consumidores de evidência acima. Para links de perfil cruzado, B2-03B1 usa os **valores** validados da lista; explicar **qual `ObservedItem`** sustentou o link é B2-03B3, e a explicação segue grosseira até lá. `links` é um atributo **sintético** do consumidor, não uma identidade de campo observado: um campo desconhecido chamado `links` continua preservado em `provenance` para auditoria, mas não pode controlar essa explicação. Nomes de campo desconhecidos seguem permitidos. Preservação de originais brutos por item e emissão de `rejected_by` pelo produtor **não** estão implementadas.
